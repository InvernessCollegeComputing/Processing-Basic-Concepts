...
void draw() {
  ...
  point(100, height/2);     // A point takes two arguments, an x coordinate and a y coordinate.
                           ...
                           // 'height' is a pre-determined variable in processing, it takes it's value from the size() function on line 6
  ...
  // The other two shapes built into Processing are Rectangles and Ellipses
  ...
  // point(), line(), rect(), and ellipse() are the only built in shapes in Processing.
  ...
  // To draw a polygon in processing we need to know the location of each vertex, so a triangle could be drawn with:
  

  beginShape();              // This function tells Processing that you are starting to draw a new polygon
    vertex(700, 300);        // This function draws a vertex, the arguments are (x coord, y coord)
    vertex(800, 300);
    vertex(750, 200);
  endShape(CLOSE);           // This funtion tells processing that you wish to finish off the polygon and draw a line between the last vertex and the first vertex
